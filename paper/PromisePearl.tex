...
[Monad instance]
[\lit{join} characterization instead of \lit{>>=}: collapse a two-layer \lit{Promise} into a single layer]
...
[pAllSettled is the simplest; describe it first]
The simplest of the parallel combiners is \lit{Promise.allSettled(iterable)}, which combines all of its input promises into a single \lit{Promise} that runs them in parallel and resolves to a list of each individual result once they are all complete
